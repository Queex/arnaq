...
keep several such `.gtf` files in parallel. In order to avoid any problems resulting from long and
complicated filenames, ARNAQ assumes each `.gtf` file occupies its own directory. The
`resources.yml` file gives this directory and ARNAQ looks for a single `.gtf` file in that location.
...
tmp_resources[3, 2] <- "gtfs/GRCm39.112" # The directory of our desired .gtf file
tmp_resources # Your locations will most likely differ
write.table(tmp_resources, "resources.yml", quote=FALSE, sep=" ", col.names=FALSE, row.names=FALSE)
```

In general, it is better to store these `.gtf` files in a common location rather alongside any
particular project, it is only done so here for demonstration putposes.
